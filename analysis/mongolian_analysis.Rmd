...
--> anticipatory coarticulation is not different across subsets
...
```
Carryover coarticulation is robust in non-harmonic sequences.
...
```
In non-harmonic sequences, the effect size of anticipatory coarticulation (opposite to direction of vowel harmony) is greater than the carryover coarticulation.
...
```
V2 terms in the full model are significant, suggesting coarticulation in the anticipatory direction. The full model is better than the null model. (however, note: null model has a singular fit).
...
For F2, null model for anticipatory coarticulation in non-harmonic sequences has a singular fit. Apart from that, patterns are identical to F1.
