...
rm(df,df_lobanov,df_har,df_nhar,df_rounding,df_ATR)
...
In the analysis that follows, we compare the full models for anticipatory and carryover coarticulation to their corresponding null models.
...
Comparing the effect size of the explanatory variable (F1 of the other vowel) in the anticipatory vs the carryover model:
...
The harmony type*F1_v2 interaction has a marginal effect in the full model, but the interaction model overall is not significanty better than the null model. Anticipatory coarticulation is not different across harmonic vs non-harmonic sequences.
...
```
The harmony type*F1_v2 interaction has a significant effect in the full model, and improves model fit. That is, carryover coarticulation is moderated by harmony type.

Overall, extent and patterns of coarticlation are different across harmonic vs. non-harmonic sequences. The next section compares coarticulation in harmonic s. non-harmonic words.
---
...
```
This shows that for both speaker and item, the irst principal component accounts for 100% of the variance.So we need a model with reduced dimensionality. Removing the correlation parameters:
